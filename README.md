...
| 🎨 UX | 单实例（V3.9.1 新） | 已开窗口时再右键 / 拖入另一份 docx，会在当前窗口替换文件，不会新开第二个窗口 |
| 🎨 UX | 紧凑顶部栏（V3.9 新） | 标题旁紧跟文件按钮（兼当前文件名提示），右上角「⚙ 设置」下拉：切换主题 / 关于 / 检查更新 |
| 🎨 UX | 复选框记忆（V3.9 新） | 所有勾选项（清洗 / 权项检查 / 删除「所述」章节 / 生成 Tab）都持久化，下次启动自动恢复 |
...
顶部为紧凑单行：标题之后紧跟「📂 文件按钮」，既是上传入口（点击 / 拖入）也是当前文件名提示；
未加载文件时显示「📂 拖入或点击选择 .docx 文件」，加载后变成「📄 文件名.docx」并以完整路径作 tooltip。
右上角「⚙ 设置」下拉包含「🌓 切换主题 / ℹ 关于 / 🔄 检查更新」三项。
通过 Windows 右键菜单打开 docx 时同样会自动载入。加载后 4 个 Tab 同时就绪，功能互不影响；
最后点「💾 文件生成」输出结果。
...
- **仅标注权利要求书 / 仅标注具体实施方式 / 仅标注说明书** — 只处理单个范围；「仅标注说明书」覆盖说明书正文（发明内容 + 具体实施方式），支持**只导入单独的说明书**（无权利要求书）后标记 / 删除
- **🧹 一键清洗** — 移除全部附图编号，恢复纯文字
...
- 错别字词库（内置 165+ 条 + 用户自定义，可增删 / 导入 / 导出）
- 结果表 5 列：`章节 | 原文片段 | 修改前 | 修改后 | 操作`；「修改后」可直接编辑
- 「忽略」列点击即跳过本条
