...
- 首次打开：自动以 claim_check.DEFAULT_BOUNDARY_BLACKLIST 填充
- 用户可增删 / 导入 / 导出
- 「恢复内置」按钮把缺失的内置词合并回当前列表
- 保存时持久化到 ~/.config/PatentMarker/boundary_blacklist.json
"""
import json
...
class BoundaryBlacklistDialog(QDialog):
    """动态截断黑名单词库编辑器"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("动态截断黑名单词库")
        self.setMinimumSize(560, 620)
        self.setModal(True)

        self._items: list = list(load_boundary_blacklist())
    ...
    def _build_ui(self):
        ...
        self.add_edit.setPlaceholderText("输入边界词后按回车或点「添加」")
    ...
    def _on_restore_defaults(self):
        builtin = get_builtin_boundary_blacklist()
    ...
    def _on_save(self):
        try:
            save_boundary_blacklist(self._items)
        except Exception as e:
            QMessageBox.critical(self, "保存失败", f"无法写入文件：\n{e}")
            return
        QMessageBox.information(
            self, "已保存",
            f"动态截断黑名单已保存，共 {len(self._items)} 条。\n下次点「开始检查」即生效。"
        )
        self.accept()
    ...
    def _on_export(self):
        ...
        path, selected_filter = QFileDialog.getSaveFileName(
            self, "导出动态截断黑名单", "boundary_blacklist.json",
